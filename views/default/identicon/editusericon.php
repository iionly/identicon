...
if ($user) {
	...
	$check = elgg_view('input/checkboxes', [
		...
		'options' => [elgg_echo('identicon:preference_checkbox') => true],
		'value' => ($user->preferIdenticon ? true : false)
	]);

	$submit = elgg_view('input/submit', ['value' => elgg_echo('save')]);

	$form = elgg_view('input/form', ['action' =>  elgg_get_site_url() . "action/identicon/userpreference?user_guid={$user->guid}", 'body' => $img . "\n" . $check . "\n<br>" . $submit]);
